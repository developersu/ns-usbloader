...
sed -z -i -e "s/BrandingText\ \"NS-USBloader\"/BrandingText\ \"NS-USBloader v$VERSIONMAJOR.$VERSIONMINOR\"\ /" misc/windows/NSIS/installer.nsi
if [ $# -eq 0 ]
  then
    sed -z -i -e "s/OutFile\ \"Installer.exe\"/OutFile\ \"Installer-$VERSIONMAJOR.$VERSIONMINOR.exe\"\ /" misc/windows/NSIS/installer.nsi
  else
    sed -z -i -e "s/OutFile\ \"Installer-$VERSIONMAJOR.$VERSIONMINOR.exe\"/OutFile\ \"Installer-legacy-$VERSIONMAJOR.$VERSIONMINOR.exe\"\ /" misc/windows/NSIS/installer.nsi
fi
